...
void GameReadyPacketHandler::handle(const Packet &packet) {
    ...
    if (sceneSystem) {
        ...
        eventQueue.push_back([sceneSystem, sceneName, levelId, network, eventManager]() {
            std::cout << "[GameReadyPacketHandler] Preparing level scene: " << sceneName << std::endl;
            
            // Check if scene already exists
            Scene* existingLevel = sceneSystem->getScene(sceneName);
            
            if (existingLevel) {
                // Scene exists - cleanup first to reset state
                if (auto* levelScene = dynamic_cast<LevelScene*>(existingLevel)) {
                    std::cout << "[GameReadyPacketHandler] Scene exists, cleaning up and reinitializing: " << sceneName << std::endl;
                    levelScene->cleanup();
                    // Cleanup sets _isInitialized to false, so setScene should trigger onInitialRender
                }
            } else {
                ...
                if (newLevelScene) {
                    ...
                    std::cout << "[GameReadyPacketHandler] Created new scene: " << sceneName << std::endl;
                }
            }
            
            // Set the scene - this should trigger onInitialRender which will reinitialize
            std::cout << "[GameReadyPacketHandler] Setting scene: " << sceneName << std::endl;
