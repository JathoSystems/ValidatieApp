...
void LevelSelector::onPlayClicked(int levelNumber) {
    ...
    for (int i = 1; i <= 10; i++) {
        ...
        if (_sceneSystem->getScene(offlineSceneName)) {
            _sceneSystem->removeScene(offlineSceneName);
        }
        if (_sceneSystem->getScene(onlineSceneName)) {
...
void LevelSelector::onOnlinePlayClicked(int levelNumber) {
    ...
    for (int i = 1; i <= 10; i++) {
        std::string offlineSceneName = "level_" + std::to_string(i);
        std::string onlineSceneName = "level_" + std::to_string(i) + "_online";
        std::string roomSceneName = "room_selection_level_" + std::to_string(i);
        if (_sceneSystem->getScene(offlineSceneName)) {
            _sceneSystem->removeScene(offlineSceneName);
        }
        if (_sceneSystem->getScene(onlineSceneName)) {
        ...
        if (_sceneSystem->getScene(roomSceneName)) {
